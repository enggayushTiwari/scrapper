...
for cat in top_10_categories:
    ...
    print(f"{cat}: The top 10% ({top_10_count} business/es) own {percentage:.1f}% of all reviews.")

print(f"\nTotal Independent: {len(df_independent)}")
print(f"Chain locations excluded: {df[df['Business Name'].isin(chains)].shape[0]}")
